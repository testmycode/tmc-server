...
describe SafeUnzipper do
  it "removes symlinks outside of the unzipped directory" do
    ...
    expect(File.exists?('out/foo/good_file')).to eq(true)
    expect(File.exists?('out/foo/bad_link')).to eq(false)
    expect(File.exists?('out/foo/bad_link2')).to eq(false)
  end
  ...
  it "preserves symlinks inside the unzipped directory" do
    ...
    expect(File.exists?('out/foo/good_file')).to eq(true)
    expect(File.exists?('out/foo/bar/good_link')).to eq(true)
