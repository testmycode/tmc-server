...
describe ExerciseCompletionStatusGenerator, "completion status" do
  ...
  it "is generated for each of the exercises" do
    ...
    completion_status.keys.sort.should == [@ex1.id, @ex2.id, @ex3.id]
  end
  ...
  describe "with awarded points and no submissions" do
    it "is correctly generated in case 1" do
      ...
      completion_status[@ex1.id].should  == nil
      completion_status[@ex2.id].should  == 100
      completion_status[@ex3.id].should  == 75
    end
    ...
    it "is correctly generated in case 2" do
      ...
      completion_status[@ex1.id].should  == 100
      completion_status[@ex2.id].should  == nil
      completion_status[@ex3.id].should  == 25
    end
  end
  ...
  describe "with only unsuccesful submissions and no awarded points" do
    it "is set to 0%" do
      ...
      completion_status[@ex1.id].should  == 0
      completion_status[@ex2.id].should  == 0
      completion_status[@ex3.id].should  == nil
    end
  end
  ...
  describe "with succesful submissions" do
    it "is correctly generated (i.e. submissions have no effect)" do
      ...
      completion_status[@ex1.id].should  == 0
      completion_status[@ex2.id].should  == 100
      completion_status[@ex3.id].should  == 50
    end
  end


  def submission(exercise, points = [])
    Factory.create(:submission, :course_id => @course.id, :exercise_name => exercise.name, :user_id => @user.id, :points => points)
  end

  def exercise(name, points)
    Factory.create(:exercise, :course_id => @course.id, :name => name, :available_points => point_objects(points))
  end
