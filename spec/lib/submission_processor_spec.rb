...
describe SubmissionProcessor do
  ...
  def expect_processing
    RemoteSandbox.should_receive(:try_to_send_submission_to_free_server).with(@sub, @sub.result_url)
  end

  def expect_no_processing
    RemoteSandbox.should_not_receive(:try_to_send_submission_to_free_server)
  end

  describe "reprocessing submissions" do

    before :each do
      @sub = Factory.create(:submission, :processed => false)
    end
    ...
    it "should not reprocess when a sandbox has received the job a reasonable time ago" do
      ...
      @sub.processing_began_at.should < @sub.processing_tried_at
      @sub.save!
      expect_no_processing
      SubmissionProcessor.new.reprocess_timed_out_submissions
    end
    ...
    context "when the submission has been reprocessed too many times" do
      ...
      it "should mark it as processed and having an error" do
        SubmissionProcessor.new.reprocess_timed_out_submissions
        @sub.reload
        @sub.should be_processed
        @sub.pretest_error.should_not be_empty
        @sub.secret_token.should be_nil
        @sub.status.should == :error
      end
    end

    context "when a sandbox receives the job" do
      before :each do
        RemoteSandbox.should_receive(:try_to_send_submission_to_free_server).and_return(true)
      end
      ...
      it "should increment the times-sent-to-sandbox counter" do
        ...
        SubmissionProcessor.new.reprocess_timed_out_submissions
        @sub.reload
        @sub.times_sent_to_sandbox.should == original_count + 1
      end

      it "should update the processing_tried_at and processing_began_at timestamps" do
        SubmissionProcessor.new.reprocess_timed_out_submissions
        @sub.reload
        @sub.processing_tried_at.should > Time.now - 5.seconds
        @sub.processing_began_at.should > Time.now - 5.seconds
      end
    end

    context "when no sandbox receives the job" do
      before :each do
        RemoteSandbox.should_receive(:try_to_send_submission_to_free_server).and_return(false)
      end
      ...
      it "should not increment the times-sent-to-sandbox counter" do
        ...
        SubmissionProcessor.new.reprocess_timed_out_submissions
        @sub.reload
        @sub.times_sent_to_sandbox.should == original_count
      end
      ...
      it "should update the processing_tried_at timestamp but not processing_began_at" do
        ...
        @sub.reload
        @sub.processing_tried_at.should > Time.now - 5.seconds
        @sub.processing_began_at.should be_nil
      end
    end
  end
end