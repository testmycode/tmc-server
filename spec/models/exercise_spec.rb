...
describe Exercise, type: :model do
  ...
  describe 'gdocs_sheet' do
    it 'should deduce gdocs_sheet from exercise name' do
      ex1 = FactoryBot.create(:exercise, name: 'ex')
      ex1.options = {}
      expect(ex1.gdocs_sheet).to eq('root')

      ex2 = FactoryBot.create(:exercise, name: 'wtf-ex')
      ex2.options = {}
      expect(ex2.gdocs_sheet).to eq('wtf')

      ex3 = FactoryBot.create(:exercise, name: 'omg-wtf-ex')
      ex3.options = {}
      expect(ex3.gdocs_sheet).to eq('omg-wtf')

      ex4 = FactoryBot.create(:exercise, name: 'omg-wtf-ex')
  ...
  describe 'associated submissions' do
    before :each do
      @exercise = FactoryBot.create(:exercise, course: course, name: 'MyExercise')
      @submission_attrs = {
        course: course,
  ...
  it 'knows which exercise groups it belongs to' do
    ...
    ex2 = FactoryBot.create(:exercise, course: course, name: 'xoo-bar-baz')
    course.reload
    another_course = FactoryBot.create(:course)
    ex3 = FactoryBot.create(:exercise, course: another_course, name: 'foo-bar-baz')
  ...
  it "should not be returnable if course's initial refresh is not completed" do
    course = FactoryBot.create(:course, initial_refresh_ready: false)
    ex = FactoryBot.create(:exercise, course: course, has_tests: true)
    expect(ex).not_to be_returnable
  end

  it "should always be submittable by administrators as long as it's returnable" do
    admin = FactoryBot.create(:admin)
    ex = FactoryBot.create(:returnable_exercise, course: course)
  ...
  it 'should not be visible to regular users if explicitly hidden' do
    user = FactoryBot.create(:user)
    ex = FactoryBot.create(:exercise, course: course, hidden: true)
  ...
  it 'should not be visible to regular users if the publish time has not passed' do
    user = FactoryBot.create(:user)
    ex = FactoryBot.create(:exercise, course: course, publish_time: Time.now + 10.hours)
  ...
  it 'should be visible to administrators even if publish time is in the future' do
    admin = FactoryBot.create(:admin)
    ex = FactoryBot.create(:exercise, course: course, publish_time: Time.now + 10.hours, hidden: false)
  ...
  it 'should be visible to administrators even if hidden' do
    admin = FactoryBot.create(:admin)
    ex = FactoryBot.create(:exercise, course: course, publish_time: Time.now - 10.hours, hidden: true)
  ...
  it "can tell if it's been reviewed for a user" do
    ...
    submission = FactoryBot.create(:submission, exercise: exercise, course: course, user: user, reviewed: true)
  ...
  it 'can tell if all review points have been given to a user' do
    ...
    FactoryBot.create(:awarded_point, course: course, user: user, name: pt2.name)
    expect(exercise).not_to be_all_review_points_given_for(user)
    FactoryBot.create(:awarded_point, course: course, user: user, name: pt3.name)
  ...
  it 'can tell which review point are missing for a user' do
    ...
    FactoryBot.create(:awarded_point, course: course, submission: submission, user: user, name: pt2.name)
